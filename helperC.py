...
def load_data(dataset="MNIST", data_dir="./data", reduced=False, normalize="channel-wise", flatten=False, device="cpu"):
...
def split_dataset(n_clients, train_ds, val_ds, alpha, sizes=None):
    ...
    for client_id in range(n_clients):
        for x in range(len(train_y_list[client_id])):
            if x == client_id:
        ...
        for x in range(len(val_y_list[client_id])):
            if x == client_id:
...
class OutputTracker():
    ...
    def get_global_outputs(self, r=-1, n_avg=None, client_id=None, sel_cli=None):
        ...
        if client_id is None:
            ...
            if client_id == "random":
                client_id = random.randint(0, self.n_clients-2)
                if(client_id >= sel_cli):
                    client_id += 1
